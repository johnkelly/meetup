defmodule Meetup.V1.OrderControllerTest do
  use Meetup.ConnCase
  ...
  test "#index" do
    ...
    assert resp["orders"] == [%{
      ...
      "subtotal"    => Decimal.to_string(order.subtotal),
      "tax"         => Decimal.to_string(order.tax),
  ...
  test "#create - valid attrs" do
    conn = post conn, v1_order_path(conn, :create), order: @valid_order_attrs
    resp = json_response(conn, 201)

    assert resp["order"]["id"]
    assert resp["order"]["item_id"] == 25
    assert resp["order"]["user_id"] == 42
    assert resp["order"]["subtotal"] == "23.0"
    assert resp["order"]["tax"]    == "6.0"
    assert resp["order"]["total"]  == "29.0"
    assert resp["order"]["inserted_at"]
    assert resp["order"]["updated_at"]
  end

  test "#create - invalid attrs" do
    conn = post conn, v1_order_path(conn, :create), order: @invalid_order_attrs
  ...
  test "#show - valid id" do
    order = create(:order)
    conn = get conn, v1_order_path(conn, :show, order.id)
    resp = json_response(conn, 200)
  ...
  test "#show - invalid id" do
    conn = get conn, v1_order_path(conn, :show, 1_000_000)
